...
const RECENT_TURNS = 6; // user+assistant pairs kept verbatim in-context
const SUMMARY_EVERY = 6; // compress once this many messages accumulate past summary
...
2. DIALOGUE DOMINANCE IS MANDATORY: 65%+ of your output MUST be spoken words inside "double quotes". You are a character talking, NOT a narrator describing. Most of your output is spoken lines.
...
[END EXAMPLE — the example is 60% spoken words. Your output must match this shape.]
`;
}
...
export async function assemble(args: {
  ...
  return {
    system: buildSystemPrompt({ ...args, facts, state }),
    messages,
    facts,
    summary,
  };
